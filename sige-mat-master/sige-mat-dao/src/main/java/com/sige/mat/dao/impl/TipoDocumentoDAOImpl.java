...
/**
 * Implementaci�n de la interface TipoDocumentoDAO.
 * @author MV
 *
 */
public class TipoDocumentoDAOImpl{
	final static Logger logger = Logger.getLogger(TipoDocumentoDAOImpl.class);
	@Autowired
	...
	public int saveOrUpdate(TipoDocumento tipo_documento) {
		if (tipo_documento.getId() != null) {
			// update
			String sql = "UPDATE cat_tipo_documento "
						+ "SET nom=?, "
						+ "est=?,usr_act=?,fec_act=? "
					+ "WHERE id=?";
			...
			return jdbcTemplate.update(sql, 
						tipo_documento.getNom(),
						tipo_documento.getEst(),
						tipo_documento.getUsr_act(),
						new java.util.Date(),
						tipo_documento.getId()); 

		} else {
			// insert
			String sql = "insert into cat_tipo_documento ("
						+ "nom, "
						+ "est, usr_ins, fec_ins) "
						+ "values ( ?, ?, ?, ?)";
	...
	public void delete(int id) {
		String sql = "delete from cat_tipo_documento where id=?";
	...
	public List<TipoDocumento> list() {
		String sql = "select * from cat_tipo_documento";
		
		//logger.info(sql);
		
		List<TipoDocumento> listTipoDocumento = jdbcTemplate.query(sql, new RowMapper<TipoDocumento>() {
	...
	public TipoDocumento get(int id) {
		String sql = "select * from cat_tipo_documento WHERE id=" + id;
		
		//logger.info(sql);
		
		return jdbcTemplate.query(sql, new ResultSetExtractor<TipoDocumento>() {

			
			public TipoDocumento extractData(ResultSet rs) throws SQLException,
					DataAccessException {
				if (rs.next()) {
					return rsToEntity(rs,"");
				}
	...
	public TipoDocumento getFull(int id, String tablas[]) {
		String sql = "select tdc.id tdc_id, tdc.nom tdc_nom  ,tdc.est tdc_est ";
	
		sql = sql + " from cat_tipo_documento tdc "; 
		sql = sql + " where tdc.id= " + id; 
		...
		return jdbcTemplate.query(sql, new ResultSetExtractor<TipoDocumento>() {
			...
			public TipoDocumento extractData(ResultSet rs) throws SQLException, DataAccessException {
				if (rs.next()) {
					TipoDocumento tipoDocumento= rsToEntity(rs,"tdc_");
	...
	public TipoDocumento getByParams(Param param) {

		String sql = "select * from cat_tipo_documento " + SQLFrmkUtil.getWhere(param);
	...
	public List<TipoDocumento> listByParams(Param param, String[] order) {

		String sql = "select * from cat_tipo_documento " + SQLFrmkUtil.getWhere(param) + SQLFrmkUtil.getOrder(order);
		
		//logger.info(sql);

		return jdbcTemplate.query(sql, new RowMapper<TipoDocumento>() {
	...
	public List<TipoDocumento> listFullByParams(TipoDocumento tipoDocumento, String[] order) {
	
		return listFullByParams(Param.toParam("tdc",tipoDocumento), order);
	
	}	
	
	
	public List<TipoDocumento> listFullByParams(Param param, String[] order) {
	...
	// funciones privadas utilitarias para TipoDocumento

	private TipoDocumento rsToEntity(ResultSet rs,String alias) throws SQLException {
		TipoDocumento tipo_documento = new TipoDocumento();

		tipo_documento.setId(rs.getInt( alias + "id"));
		tipo_documento.setNom(rs.getString( alias + "nom"));
		tipo_documento.setEst(rs.getString( alias + "est"));
