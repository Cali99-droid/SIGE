	...
	var fncExito = function(data){
		...
		$('#mat_seccion_sugerida-tabla').dataTable({
	         ...
	         columns : [ 
	     	           ...
	     	           {"title": "Seccion Sugerida", "render":function ( data, type, row,meta ) {
	     	        	  var combo; 
...
function onchangeCombo( field){
	...
	swal(
			{
				title : "Esta seguro?",
				text :'Se actualizará el aula sugerida',
				type : "warning",
				showCancelButton : true,
				confirmButtonColor : "rgb(33, 150, 243)",
				cancelButtonColor : "#EF5350",
			...
			function(isConfirm) {
				if (isConfirm) {
					...
					_post('api/seccionSugerida/grabar',param, function(data){
					...
					swal({
						title : "Cancelado",
						text : "No se ha actualizado el aula sugerida",
						confirmButtonColor : "#2196F3",
						type : "error"
					});
