...
$(function(){
	...
	_get('api/familiar/listarHijosMatriculadosNoTrasladados',
			function(data){
			...
			lista.children().each(function () {
				...
				$(this).find('#usuario').html(alu.usuario);
				$(this).find('#pass').html(alu.pass_google);
...
function restablecer(obj){
		//e.preventDefault();
	 	...
	 	//var usuario = $(obj).parent().attr('usuario');
		var usuario = $(obj).parent().parent().find('#usuario').text();;
...
function activar(obj){
		...
		var usuario = $(obj).parent().parent().find('#usuario').text();;
		_post(
			"api/matricula/activarCuentasGooglexUusuario/"+usuario,
			$('#frm-traslado')
					.serialize(),
			function(data) {
