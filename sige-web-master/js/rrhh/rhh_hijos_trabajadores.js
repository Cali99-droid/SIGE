...
var fncExito = function(data){
		...
		$('#mat_reporte_hijos_matriculados').dataTable({
			 data : data,
			 aaSorting: [],
			 destroy: true,
			 pageLength: 1000,
	         ...
	         columns : [ 
					  ...
					  // {"title": "Fecha Matrícula", "data" : "fec_mat"},
					   {"title": "Fecha Matrícula","render":function ( data, type, row ) {
	    	        	  /* if (row.parentesco==null)
	    	        		   return 'Mismo Alumno';
	    	        	   else */
	    	        		   return _parseDate(row.fec_mat);
	    	        	  }
					   },
					   {"title": "Mensualidad","render":function ( data, type, row ) {
	    	        	  /* if (row.parentesco==null)
	    	        		   return 'Mismo Alumno';
	    	        	   else */
	    	        		   return 'S/.'+row.monto;
...
$('#frm-reporte-hijos').on('submit',function(event){
		event.preventDefault();
		
	
		var param={ id_anio:$('#_id_anio').text(), id_gir:$('#id_gir').val(), tip_con:$('#tip_con').val(), id_suc:$('#id_suc').val()};
		return _get('api/trabajador/listarHijosTrabajadores',fncExito, param);
	});
