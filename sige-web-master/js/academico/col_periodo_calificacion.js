	 ...
	 */
	 ...
	 columns : [ 
			...
			/*{"title":"Acciones", "render": function ( data, type, row ) {
				...
				listar_calificacion_periodo(grad_seleccionado, $("#_id_anio").text(), $("#id_cpu").val(),nom_grad_seleccionado,id_gir);
...
function areas_modal(id_anio, id_gra){
	...
	var onShowModal = function(){
		...
		var param2={id_gra:id_gra, id_anio:id_anio, id_cpu:$('#id_cpu').val(), id_gir:$('#id_gir').val()};
		_get('api/perUni/detallePeriodoCalificacion',
				function(data){
				var id_tca=data.id_tca;
				$('#col_area_anio_dc-frm #id_tca').val(id_tca).change();
				$('#col_area_anio_dc-frm #id_pro_per').val(data.id_pro_per).change();
				$('#col_area_anio_dc-frm #id_pro_anu').val(data.id_pro_anu).change();
		}, param2);
		
		var param ={id_anio: id_anio, id_gra:id_gra, id_gir:$('#id_gir').val()};
...
function listar_calificacion_periodo(id_gra, id_anio, id_cpu, nom_grad_seleccionado, id_gir){
	...
	var param={id_gra:id_gra, id_anio:id_anio, id_cpu:id_cpu, id_gir:id_gir};
	_get('api/perUni/detallePeriodoCalificacion',
			function(data){
			var id_tca=data.id_tca;
