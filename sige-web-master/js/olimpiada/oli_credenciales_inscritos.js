...
function onloadPage(params) {
	...
	$('#frm-reporte').on('submit', function(event){
		...
		//alert();
		if ($(this).valid())
		_GET({url:'api/reporte/inscritos',
				...
				console.log(data);  
				  var columns = [ 
						...
						{"title": "Seleccionar", "render":function ( data, type, row,meta ) {
							...
							return "<input type='checkbox' id='id_ins"+row.id+"' data-tipo='" + tipo + "' class='id_ins' name ='id_ins' value='" + row.id+ "'   />";							
						}},
				 ...
				 /* if(_usuario.roles.indexOf(_ROL_ADMINISTRADOR)>-1 || _usuario.roles.indexOf(_ROL_ADMIN_OLI)>-1){
				  ...
				  $('#tabla-reporte').dataTable({
					    ...
					    "initComplete": function( settings ) {
				        	...
				        	$("<span><a href='#' onclick='descargarCarnetVarios(event)'> <i class='fa fa-file-excel-o'></i>Descargar Carnets</a>&nbsp;<input name='Todos' type='checkbox' value='' class='check_todos'/>Seleccionar todos</input></span>").insertBefore($("#_paginator"));
				        	 _dataTable_total(settings,'monto');
							  $(".check_todos").click(function(event){
								if($(this).is(":checked")) {
								//$(".ck:checkbox:not(:checked)").attr("checked", "checked");
								$("#tabla-reporte input[name='id_ins']").each(function() {
...
function descargarCarnetVarios(e){
	var id_ins=[];
	var tipo=[];
	...
	_download(_URL_OLI+'api/public/inscripcionInd/imprimirCarnetsVarios?tipo='+tipo+'&id_ins='+id_ins,'carnets_olimpiada.pdf');	
}
