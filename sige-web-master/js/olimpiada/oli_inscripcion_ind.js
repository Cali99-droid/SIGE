...
function onloadPage(params){
	...
	if(tip=='I'){
		...
		_GET({url:'api/public/config/ontenerDatosConcurso',
			...
			success:function(data){
				...
				if(data!=null){
					...
					$("#col").attr("readonly","readonly");
					$("#titulo").text('MODALIDAD INTERNO');
					$("#descripcion").text('Sólo alumnos del Colegio '+data.colegio);
		...
		$('#ape_pat').focus(function() {
			var param={dni:$('#nro_dni').val(),id_anio:id_anio,id_oli:$("#id_oli").val()};
			if($('#nro_dni').val()!=null && $('#nro_dni').val()!=''){
		...
		$("#id_ti").val(2);
		$("#titulo").text('MODALIDAD LIBRE');
		$("#descripcion").text('Alumnos serán premiados, pero no suman puntaje para su colegio');
		_llenar_combo({
			tabla:'cat_gestion',
			combo:$('#id_cg'),
			context:_URL_OLI,
			...
			if($('#id_cg').val()!=null && $('#id_cg').val()!=''){
				...
				_GET({url:'api/public/costo/obtenerCostoInscripcion',
					...
					success:function(data){
						console.log(data);
						if(data==null){
							_alert_error('No existe costo configurado!!');
							$('#btn-grabar').attr('disabled','disabled');
						} else{
							$("#oli_inscripcion_ind-frm #id_oc").val(data.id);
							$('#btn-grabar').removeAttr("disabled");
		 ...
		 if ($("#oli_inscripcion_ind-frm").valid()){
				if(dni.length<8){
					...
					$("#nro_dni").focus();
					 ...
					 var param={dni:$('#nro_dni').val(),id_oli:$('#id_oli').val()}
					 $("#id_dist").removeAttr('disabled');
						_GET({url:'api/public/inscripcionInd/existeAlumno',
							params: param,
							context:_URL_OLI,
							success:function(data){
								if(data==true){
									_alert_error('El número de DNI ingresado ya existe, verifíquelo');
									 $('#nro_dni').focus();
								} else{
										 $('#btn-grabar').attr('disabled','disabled');
											var id_ti=$('#oli_inscripcion_ind-frm #id_ti').val();
											var col_org=$('#oli_inscripcion_ind-frm #col_org').val();
											var col=$('#oli_inscripcion_ind-frm #col').val();
											if(id_ti==2 && col_org==col){
												$('#btn-grabar').attr('disabled','disabled');
												_alert_error('Usted debe inscribirse como alumno interno, al pertenecer al colegio organizador!!');
											}
											
											$('#oli_inscripcion_ind-frm #id').val('');
											
											if(_tip=='I'){
												$('#oli_inscripcion_ind-frm #id_cg').removeAttr('disabled');
												$('#oli_inscripcion_ind-frm #id_dep').removeAttr('disabled');
												$('#oli_inscripcion_ind-frm #id_dist').removeAttr('disabled');
												$('#oli_inscripcion_ind-frm #id_pro').removeAttr('disabled');
												$('#oli_inscripcion_ind-frm #id_niv').removeAttr('disabled');
												$('#oli_inscripcion_ind-frm #id_og').removeAttr('disabled');
											}
											
											_POST({form:$('#oli_inscripcion_ind-frm'),
												  context:_URL_OLI,
												  msg_type:'notification',
												  msg_exito : 'PARA FINALIZAR LA INSCRIPCION, DEBERÁ REALIZAR EL PAGO EN SECRETARIA.',
												  success:function(data){
													    //DESCARGAR 
													    _download(_URL_OLI+'api/public/inscripcionInd/imprimirCarnet?id_ins='+data.result+'&tipo=I',dni + '_olimpiada.pdf');
													  ...
													  //$('#oli_inscripcion_ind-frm').trigger('reset');
													    ...
													    if($("#id_ti").val()==1){
													    	_llenar_combo({
													    	   ...
													    	   $('#id_pro').attr("disabled","disabled");
													    	   $('#id_dist').attr("disabled","disabled");
													    	   //$('#oli_inscripcion_ind-frm #id_og').reset();
													    } else  if($("#id_ti").val()==2){
													    	//$('#id_cg').change();
													    	$('#oli_inscripcion_ind-frm #col').val('');
													    	//$('#oli_inscripcion_ind-frm #id_og').change();
													    }
