...
function reglas_listar_tabla(){

	var param = {id_mod:$('#ges_reglas_negocio-tabla').data('id_padre'), id_emp:1};
	console.log(param);
	_get('api/reglasNegocio/listarReglasNegocio/',
			function(data){
			console.log(data);
				$('#ges_reglas_negocio-tabla').dataTable({
					 data : data,
					 aaSorting: [],
					 destroy: true,
					 //bLengthChange: false,
			         ...
			         columns : [ 
							...
							{"title":"Regla", "data" : "nom"},
							{"title":"Código", "data" : "cod"}, 
							{"title":"Valores", "render": function ( data, type, row ) {
								if(row.tipo_html=='valor'){
									return "<div class='regla_negocio'><input type='hidden' id='id_par' name='id_par' value='"+row.id+"' /></input><input type='text' id='val' name='val' value='"+row.val+"' class='form-control input-sm'/></div>";
								}else if(row.tipo_html=='flag'){
									var checked = (row.val!=0) ? ' checked ': ' ';									
										return "<div class='regla_negocio'><input type='hidden' id='id_par' name='id_par' value='"+row.id+"' /><input type='checkbox'"+checked+" id='val' name='val' value='"+row.val+"' class='form-control-styled input-sm' onclick='seleccionarCheck(this)'/></div>";
									
								}else if(row.tipo_html=='fecha'){
									return "<div class='regla_negocio'><input type='hidden' id='id_par' name='id_par' value='"+row.id+"' /><input type='text' id='val' name='val' value='"+row.val+"' class='form-control input-sm daterange-single' /></div>";
								}
							}}
				    ],
...
$('#btn-grabar').on('click',function(){
	console.log($('#ges_reglas_negocio'));
	var json_modulo={};
	var reglas=[];
	
	$(".regla_negocio").each(function (index, value) {
		var regla={};
		regla.id_par=$(this).find("#id_par").val()
		regla.val=$(this).find("#val").val();
		reglas.push(regla);
	});
	json_modulo.parametroReq=reglas;	
