...
function onloadPage(params) {
	...
	$('#id_area').on('change',function(event) {
		...
		} else if (_usuario.roles.indexOf(_ROL_COORDINADOR_TUTOR)>-1 || _usuario.roles.indexOf(_ROL_ADMINISTRADOR)>-1 || _usuario.roles.indexOf(_ROL_COORDINADOR_AREA>-1)){
			param={id_caa:$('#id_area').val(), id_au:$('#id_au').val()};
		} else {
		...
		_llenarComboURL('api/areaAnio/listarCursosDCNXAreaxGradoCombo',
			$('#id_cur'), null, param, function() {
				...
				if($('#id_cur').val()==null){
					...
					if (_usuario.roles.indexOf(_ROL_COORDINADOR_NIVEL)>-1 || _usuario.roles.indexOf(_ROL_COORDINADOR_TUTOR)>-1 || _usuario.roles.indexOf(_ROL_ADMINISTRADOR)>-1 || _usuario.roles.indexOf(_ROL_COORDINADOR_AREA>-1)){
	...
	$('#id_au').on('change',function(event) {
		...
		} else if (_usuario.roles.indexOf(_ROL_COORDINADOR_AREA)>-1){
			var param={id_anio:$("#_id_anio").text(), id_gra:$('#id_au option:selected').attr("data-aux1"), id_tra:_usuario.id_tra, id_gir:$('#id_gir').val()};
			_llenarComboURL('api/areaAnio/listarAreasxCoordinadorAreaDCNGradoCombo',
			$('#id_area'), null, param, function() {
				$('#id_area').change();
				//$('#btn-buscar').click();
			});
	...
	var param2={id_anio:$("#_id_anio").text(), id_gra:$('#id_au option:selected').attr("data-aux1"), id_tra:_usuario.id_tra, id_gir:$('#id_gir').val()};
		if (_usuario.roles.indexOf(_ROL_COORDINADOR_NIVEL)>-1){
		...
		} else if (_usuario.roles.indexOf(_ROL_COORDINADOR_TUTOR)>-1 || _usuario.roles.indexOf(_ROL_ADMINISTRADOR)>-1){
			...
			var param ={id_anio:$("#_id_anio").text(), id_tra:_usuario.id_tra, id_gir:$('#id_gir').val()};
			_llenarComboURL('api/aula/listarAulasxCoordinadorArea', $('#id_au'),null, param, function() {
			$('#id_au').change();
			});
		} else {
	...
	if (_usuario.roles.indexOf(_ROL_COORDINADOR_NIVEL)>-1){
		...
		_llenarComboURL('api/trabajador/listarGirosNegocioxCoordinadorArea/'+id_tra+'/'+id_anio,$('#id_gir'),null,null,function(){$('#id_gir').change();});
	} else {	
		_llenarCombo('ges_giro_negocio',$('#id_gir'), null, null,function() {
			$('#id_gir').change();
		});	
...
//este es, se qued
		...
		if($('#id_cur').val()=="T"){
			...
			if(com!='' && capacidades[key].com_nom!=com)
				td =  td + '<th bgcolor="#AED6F1">PROM.</th>';
			//td = td + '<th data-popup="tooltip" title="' +  capacidades[key].nom + '" data-placement="top">C' + i + '</th>';
			td = td + '<th colspan="' + (capacidades[key].count_cap) + '" bgcolor="#D5F5E3" data-popup="tooltip" title="' +  capacidades[key].capacidad + '" data-placement="top">C' + capacidades[key].orden + '</th>';
				...
				//	var id_cap = notas[keyNotas].id_cap;
					...
					//if(cursos[y].id_cua==notasCur[keyNotas].id_cua && cursos[y].id_com==notasCur[keyNotas].id_com){
						...
						if(tip_cali=="CUALI"){
							...
							tdsNotas = tdsNotas  + "<td><label style='font-size: 9pt; font-weight: bold; color: "+color+"; text-align: center;' >"+nota_value+"</label></td>";
							...
							tdsNotas = tdsNotas  + "<td><label style='font-size: 9pt; font-weight: bold; color: "+color+"; text-align: center;' ><font style='text-align: center;'>"+nota_value+"</font></label></td>";
						}	
					//}		 
				}
				//}	
