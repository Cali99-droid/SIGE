...
$(function(){
	...
	if (_usuario.roles.indexOf(_ROL_COORDINADOR_NIVEL)>-1 || _usuario.roles.indexOf(_ROL_AUXILIAR)>-1){
	...
	$('#form_registro_asistencia').on('submit',function(event){
		event.preventDefault();
		return _POST({  url:'api/codigoBarras/registrarAsistencia',
						form: $(this),
						success:function(data){
					console.log(data.result);
					$('#lect_ing').val('');
					if(data.result.existe==0){
						$('#resultados').css('display','block');
						if(data.result.tardanza==0){
						$('#div-res').addClass('panel-success');
						$('#reg_asis').css('color', '#1E8449');}
						else if(data.result.tardanza==1){
						$('#div-res').addClass('panel-warning');
						$('#reg_asis').css('color', 'red');}
						
						$('#aula').css('display','block');
						$('#reg_asis').css('display','block');
						$('#hora').css('display','block');
						$('#alumno').text(data.result.alumno);
						$('#aula').text(data.result.aula);
						$('#reg_asis').text(data.result.registro_asistencia);
						$('#hora').text(data.result.hora);
						
					} else if(data.result.existe==1){
						$('#resultados').css('display','block');
						$('#div-res').addClass('panel-info');
						$('#alumno').text(data.result.mensaje);
						$('#aula').css('display','none');
						$('#reg_asis').css('display','none');
	...
	/*$('#btn-registrar').on('submit',function(event){
		event.preventDefault();
		...
		/*_POST({form:$('#form_registro_asistencia'),
			  context:_URL,
			  msg_type:'notification',
			 // msg_exito : 'Se ha registrado correctamente',
			  success:function(data){
					console.log(data.result);
					$('#lect_ing').val('');
					if(data.result.existe==0){
